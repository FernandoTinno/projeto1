...
import random
def cadastro_prof(nome,data_dia,data_mes,data_ano,sexo,endereco,tel,email,disciplina):
    ...
    codfinal_prof = cod_profstr + nome[0]
    #queria colocar / na data de nascimento mas não sabia fazer, então improvisei 
    ficha_aluno = {'nome': nome, 'data de nascimento' : f'{data_dia}/{data_mes}/{data_ano}', 'sexo' : sexo, 'endereco' : endereco, 'telefone' : tel, 'email' : email, 'disciplina' : disciplina , 'Codigo do professor' : codfinal_prof}
...
resultado = cadastro_prof(nome,data_dia,data_mes,data_ano,sexo,endereco,tel,email,disciplina)
